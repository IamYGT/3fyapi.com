...
if($_POST) {
	...
	foreach($tablo as $sira => $rows){
		...
		if(in_array($rows["type"], $normal_kayit)) {
			...
			$vt_row[] = "?";
			$asilkontrol=seo($_POST[$rows["options"]]);
			$kontrol=seo($_POST[$rows["options"]]);
			$sayi=0;
			while(true){
				if($db->VeriOkuTek("urunler_items", "url", "url", $kontrol)==$kontrol){
					$kontrol=$asilkontrol.$sayi;
					$sayi=$sayi+1;
				}
				else{
						$vt_value[] = $kontrol ;
                        	...
                        	<div class="col-lg-12">
                        		<div class="card-box">
                        			<div class="row">
			                        	...
			                        	<div class="col-sm-4">
			                        		 <a href="<?=$new_link;?>action=add" class="btn btn-default btn-md waves-effect waves-light m-b-30"><i class="md md-add"></i> Ürün Ekle</a>
			                        	</div>
			                        </div>
